...
export const createMaterialTransaction = async (req, res) => {
    try {
        const { date, data_type, type, partner, challan_no, formItems, project_id } = req.body;
        ...
        res.status(201).json({ message: "Material Transaction created", transaction });
    } catch (error) {
...
export const updateMaterialTransaction = async (req, res) => {
    try {
        ...
        if (Array.isArray(formItems)) {
            ...
            await MaterialTransactionForm.destroy({ where: { material_transaction_id: id } });
